...
import '../../main.dart';
import '../../model/sliderelectionmodel/slider_election_city/slider_election_city.dart';
import '../../model/votebyuser.dart';
import 'package:voter_multi_app/custom/const.dart';

class CityElectionView extends StatefulWidget {
  const CityElectionView({super.key});

  @override
  _CityElectionViewState createState() => _CityElectionViewState();
}

class _CityElectionViewState extends State<CityElectionView> {
  ...
  // -------------------------------- Fetches candidates and their vote counts for each group  ------------------------------- //
  ...
  //   });
  //
  //   final url = 'https://vote.nextgex.com/api/elected_candidate_city';
  ...
  //
  //         print("Hello groupId ${groupId}");
  //
  //         // Set loading state for each group
  //         setState(() {
  ...
  //         });
  //
  //         // Remove loading state for each group
  //         setState(() {
  ...
  // --------------------------------- Here User Can  Vote for a Candidate group ------------------------------ //
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // groupHasVoted[groupKey] = true;  // Mark as voted
          groupLoadingStates[groupKey] = false; // Hide loader
        });
  ...
  // ------------------------------ It's Help to  Fetch City Candidates ------------------------------ //
  ...
  // -------------------------------- Here Fetch the User Profile Data ------------------------ //
  ...
  // ------------------------ Here Fetch the User Data means Like today Votes, today Election ------------------------ //
  ...
  // ------------------- SharedPreferences For Store the Bearer Token ------------------- //
  ...
  Future<List<ElectedCandidates>> fetchCandidates() async {
    ...
    final response = await http.get(
      Uri.parse(
          'https://vote.nextgex.com/api/elected_candidate_city_image_data'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json'
      },
  ...
  Widget buildCandidatesContainer(List<Candidate> candidates, String groupKey) {
    if (groupHasVoted[groupKey] == true) {
      return const SizedBox
          .shrink(); // Return an empty container if the group has been voted and removed
    }
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          margin: const EdgeInsets.symmetric(
                              horizontal:
                                  4), // Added a slight margin for spacing
                          decoration: BoxDecoration(
                            color: const Color(0xffa0cf1a),
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(
                            ...
                            children: [
                              ...
                              CircleAvatar(
                                radius: MediaQuery.of(context).size.width *
                                    0.04, // Slightly larger dynamic CircleAvatar
                              ...
                              const SizedBox(
                                  height:
                                      8), // Adjusted spacing for better layout
                              Flexible(
                                child: Text(
                                  candidate.userName,
                                  maxLines: 1,
                                  ...
                                  style: GoogleFonts.habibi(
                                    ...
                                    fontSize:
                                        MediaQuery.of(context).size.width *
                                            0.025, // Adjusted dynamic font size
                                  ),
                                ),
                              ),
                              const SizedBox(height: 10),
                              if (votedCandidateIndices[groupKey] == null)
                                InkWell(
                                  onTap: () {
                                    castVote(candidate.id, "city", groupKey);
                                  },
                                  child: Container(
                                    height: MediaQuery.of(context).size.height *
                                        0.03, // Slightly increased button height for better touch target
                                    width: MediaQuery.of(context).size.width *
                                        0.13, // Increased button width for better tap area
                                    decoration: BoxDecoration(
                                      border: Border.all(
                                        color: Colors.black,
                                        width: 2,
