...
class CityHistoryWin extends StatefulWidget {
  const CityHistoryWin({super.key});

  @override
  State<CityHistoryWin> createState() => _CityHistoryWinState();
}

class _CityHistoryWinState extends State<CityHistoryWin> {
  ...
  Future<TodayVoterResponse> fetchVotingHistory() async {
    ...
    final requestBody = jsonEncode({'type': 'city'});
    ...
    if (response.statusCode == 200) {
      ...
      throw const Text('No voting history Found');
    }
  }
      ...
      body: FutureBuilder<TodayVoterResponse>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            var detail = snapshot.data!.todayVoter
                .expand((voter) => voter.details)
                .toList();
