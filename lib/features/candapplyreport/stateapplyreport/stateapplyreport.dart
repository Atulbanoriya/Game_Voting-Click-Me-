...
class StateApplyReport extends StatefulWidget {
  const StateApplyReport({super.key});

  @override
  State<StateApplyReport> createState() => _StateApplyReportState();
}

class _StateApplyReportState extends State<StateApplyReport> {
  ...
  Future<List<Map<String, dynamic>>> fetchHistoryData() async {
    ...
    final response = await http.get(
      Uri.parse('https://vote.nextgex.com/api/elected_candidate_total_state'),
      headers: {
  ...
  void filterDataByDate(String selectedDate) {
    setState(() {
      filteredData = filteredData.where((item) {
        return item['created_at']?.startsWith(selectedDate) ?? false;
      }).toList();
    });
  }
                ...
                return SingleChildScrollView(
                  ...
                  child: ConstrainedBox(
                    constraints: BoxConstraints(
                      minWidth: constraints.maxWidth, // Constrain width
                      maxWidth: constraints.maxWidth, // Constrain width
                    ),
                    child: SingleChildScrollView(
                      scrollDirection: Axis.horizontal,
                      child: SizedBox(
                        width: constraints.maxWidth, // Set the width of the table
                        child: PaginatedDataTable(
                          ...
                          source: StateApplyReportDataSource(filteredData),
...
class StateApplyReportDataSource extends DataTableSource {
  final List<Map<String, dynamic>> data;

  StateApplyReportDataSource(this.data);
  ...
  DataRow? getRow(int index) {
    ...
    if (electStartTime != null && now.isBefore(electStartTime.add(Duration(hours: 1)))) {
      winLoseText = 'Pending';
    } else {
      if (winLoseStatus == 0) {
        winLoseText = 'Pending';
    ...
    (electStartTime == null || now.isAfter(electStartTime.add(Duration(hours: 1))))
        ? 'Closed'
        : 'Pending';


    final String winPoint = (winLoseStatus == 1) ? '4000' : '0';
    ...
    return DataRow(
      cells: [
        ...
        DataCell(
            Center(child: Text(myStatusText))), // Added missing cell for Status
      ],
    );
  }
